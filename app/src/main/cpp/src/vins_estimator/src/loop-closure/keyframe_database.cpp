...
void KeyFrameDatabase::optimize4DoFLoopPoseGraph(int cur_index, Eigen::Vector3d &loop_correct_t, Eigen::Matrix3d &loop_correct_r)
{
	...
	//Quaterniond q_array[max_length];   //solomon modified for some armeabi-v7a ndk compiler issue
	boost::shared_array<Quaterniond> q_array(new Quaterniond[max_length]);
	...
	options.max_solver_time_in_seconds =SOLVER_TIME * 50;// SOLVER_TIME * 3;
