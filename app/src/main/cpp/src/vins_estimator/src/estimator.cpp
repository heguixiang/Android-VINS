...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    acc_0 = linear_acceleration;
    gyr_0 = angular_velocity;
    //LOGD("acc_0: %lf, %lf ,%lf.  gyro_0: %lf, %lf, %lf ", acc_0(0), acc_0(1), acc_0(2), gyr_0(0), gyr_0(1), gyr_0(2));
}
...
void Estimator::processImage(const map<int, vector<pair<int, Vector3d>>> &image, const std_msgs::Header &header)
{
  ...
  //  ROS_DEBUG("Adding feature points %lu", image.size());
    ...
    LOGD("this frame is--------------------%s", marginalization_flag ? "reject" : "accept");
   // ROS_DEBUG("%s", marginalization_flag ? "Non-keyframe" : "Keyframe");
   // ROS_DEBUG("Solving %d", frame_count);
   LOGD("number of feature: %d", f_manager.getFeatureCount());
    ...
    if (solver_flag == INITIAL)
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            {
            LOGD(" ===========before result = initialStructure===============");
               result = initialStructure();
            LOGD("================= after result = initialStructure==============");
...
bool Estimator::initialStructure()
{
  ...
  */
    ...
    {
     LOGD("Not enough features or parallax; Move device around");
        return false;
    }

	LOGD("relative_R : ");
	LOGD("relative_R : %f            %f               %f",  relative_R(0,0), relative_R(0,1), relative_R(0,2));
	LOGD("relative_R : %f            %f               %f",  relative_R(1,0), relative_R(1,1), relative_R(1,2));
	LOGD("relative_R : %f            %f               %f",  relative_R(2,0), relative_R(2,1), relative_R(2,2));
	LOGD("relative_T : %f   %f   %f", relative_T.transpose()(0), relative_T.transpose()(1), relative_T.transpose()(2) );
	LOGD("L: %d", l);
    ...
    for (int i = 0; frame_it != all_image_frame.end( ); frame_it++)
    {
        ...
        if(pts_3_vector.size() < 6)
        {
          ...
          LOGD("Not enough points for solve pnp !");
            return false;
        }
    ...
    {
    LOGD("misalign visual structure with IMU");
        return false;
    }
